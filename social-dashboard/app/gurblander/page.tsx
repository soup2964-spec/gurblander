...
export default function GurblanderPage() {
  ...
  async function onWash() {
    ...
    try {
      ...
      const res = await fetch("/api/gurblander", { method: "POST", body });
      const payload = (await res.json()) as WashResponse & { error?: string };
      if (!res.ok) throw new Error(payload.error ?? "Gurblander processing failed");
      setResult(payload);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Gurblander processing failed");
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="mx-auto min-h-screen w-full max-w-3xl px-4 py-10">
      <section className="rounded-xl border border-rose-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-2xl font-semibold text-rose-900">Gurblander Dashboard</h1>
